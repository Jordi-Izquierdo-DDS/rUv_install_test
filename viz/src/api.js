...
export function registerRoutes(app) {
  ...
  // ─── 4. GET /api/patterns — Live Ruflo learning stores ──────
  //   Reads from the REAL learning system: .swarm/memory.db
  ...
  app.get('/api/inspect/hm-tiers', (req, res) => {
    try {
      const dbPath = join(getDataRoot(), '.swarm', 'memory.db');
      if (!existsSync(dbPath)) return res.json({ ok: true, tiers: {}, note: 'memory.db not found' });
      const Database = require('better-sqlite3');
      const db = new Database(dbPath, { readonly: true });
      const tiers = {};
      try {
        const rows = db.prepare("SELECT tier, count(*) as cnt FROM hierarchical_memory GROUP BY tier").all();
        for (const r of rows) tiers[r.tier] = r.cnt;
      } catch {}
      const total = Object.values(tiers).reduce((s, v) => s + v, 0);
      db.close();
      res.json({ ok: true, tiers, total });
    } catch (e) {
      res.json({ ok: false, error: e.message });
    }
  });

  // ─── 4d-ii. GET /api/inspect/patterns-stats — Patterns.db short/long counts + top patterns ──

  app.get('/api/inspect/patterns-stats', (req, res) => {
    try {
      const dbPath = join(getDataRoot(), '.claude-flow', 'learning', 'patterns.db');
      if (!existsSync(dbPath)) return res.json({ ok: true, shortTerm: 0, longTerm: 0, note: 'patterns.db not found' });
      const Database = require('better-sqlite3');
      const db = new Database(dbPath, { readonly: true });
      let shortTerm = 0, longTerm = 0, topLong = [], topShort = [];
      try { shortTerm = db.prepare('SELECT count(*) as c FROM short_term_patterns').get()?.c || 0; } catch {}
      try { longTerm = db.prepare('SELECT count(*) as c FROM long_term_patterns').get()?.c || 0; } catch {}
      try { topLong = db.prepare('SELECT strategy, domain, usage_count, quality FROM long_term_patterns ORDER BY quality DESC LIMIT 10').all(); } catch {}
      try { topShort = db.prepare('SELECT strategy, domain, quality FROM short_term_patterns ORDER BY quality DESC LIMIT 10').all(); } catch {}
      db.close();
           // SONA JSON augmentation (audit v3 §5 R3 — post-Phase-2 patterns land in
      // post-Phase-2 patterns land in .ruvector/sona-state.json, not in legacy SQL).
      let sonaPatternCount = 0;
      let sonaTopPatterns = [];
      let sonaTypeBreakdown = {};
      try {
        const sonaPath = join(getDataRoot(), ".ruvector", "sona-state.json");
        if (existsSync(sonaPath)) {
          const sona = JSON.parse(readFileSync(sonaPath, "utf-8"));
          const patterns = Array.isArray(sona.patterns) ? sona.patterns : [];
          sonaPatternCount = patterns.length;
          for (const pat of patterns) {
            const t = pat.pattern_type || "General";
            sonaTypeBreakdown[t] = (sonaTypeBreakdown[t] || 0) + 1;
          }
          sonaTopPatterns = patterns
            .map(pat => ({
              id: pat.id,
              pattern_type: pat.pattern_type || "General",
              avg_quality: pat.avg_quality,
              access_count: pat.access_count || 0,
              cluster_size: pat.cluster_size || 1,
        ...
        // SONA store optional — never fail the legacy SQL response on its absence
      }
      res.json({ ok: true, shortTerm, longTerm, topLong, topShort, sonaPatternCount, sonaTopPatterns, sonaTypeBreakdown });
    } catch (e) {
      res.json({ ok: false, error: e.message });
    }
  });

  // ─── 4d-iii. GET /api/inspect/ewc-fisher — EWC Fisher matrix status ──

  app.get('/api/inspect/ewc-fisher', async (req, res) => {
    try {
      const bridgePath = join(getDataRoot(), 'node_modules', '@claude-flow', 'cli', 'dist', 'src', 'memory', 'ewc-consolidation.js');
      if (!existsSync(bridgePath)) return res.json({ ok: true, note: 'ewc-consolidation.js not found' });
      const ewcMod = await import('file://' + bridgePath);
      if (!ewcMod.getEWCConsolidator) return res.json({ ok: true, note: 'getEWCConsolidator not available' });
      const consolidator = await ewcMod.getEWCConsolidator();
      const stats = consolidator?.getStats ? consolidator.getStats() : {};
      const fisherSize = consolidator?.fisherMatrix ? Object.keys(consolidator.fisherMatrix).length : (stats.fisherDimensions || 0);
      res.json({ ok: true, fisherDimensions: fisherSize, stats });
    } catch (e) {
      res.json({ ok: false, error: e.message });
    }
  });

  // ─── 4d-iv. GET /api/inspect/skills — Skills list from ReasoningBank/LearningSystem ──

  app.get('/api/inspect/skills', (req, res) => {
    try {
      const dbPath = join(getDataRoot(), '.swarm', 'memory.db');
      if (!existsSync(dbPath)) return res.json({ ok: true, skills: [], note: 'memory.db not found' });
      const Database = require('better-sqlite3');
      const db = new Database(dbPath, { readonly: true });
      let skills = [];
      try {
        skills = db.prepare("SELECT name, success_rate, attempts, created_at FROM skills ORDER BY success_rate DESC LIMIT 20").all();
      } catch {
        // skills table may not exist
        try {
          skills = db.prepare("SELECT task_type as name, success_rate, uses as attempts FROM reasoning_patterns ORDER BY success_rate DESC LIMIT 20").all();
        } catch {}
      }
      db.close();
  ...
  // ─── 4d-v. POST /api/session/drain — OBSOLETE (queue removed in v4+) ──

  app.post('/api/session/drain', (req, res) => {
    res.json({ ok: true, drained: 0, note: 'Queue removed in v4+. Hooks are direct, no queue.' });
  });

  // ─── 4d-vi. POST /api/session/end-sim — Simulate session end (drain + all 7 layers) ──

  app.post('/api/session/end-sim', async (req, res) => {
    const results = [];
    try {
      // Step 1: Session end via MCP
      try {
        const r = await vizMcpCall('agentdb_session-end', {});
        results.push({ step: 'session-end', ok: true, via: 'mcp', output: JSON.stringify(r).slice(0, 300) });
      } catch (e) {
        results.push({ step: 'session-end', ok: false, error: e.message });
      }

      // Step 2: Consolidate all layers via MCP
      try {
        const r = await vizMcpCall('agentdb_consolidate', {});
        results.push({ step: 'consolidate-all', ok: true, via: 'mcp', output: JSON.stringify(r).slice(0, 300) });
      } catch (mcpErr) {
        // Fallback to cold bridge
        try {
          const bridge = await getBridge();
          if (typeof bridge.bridgeRunAllLayers === 'function') {
            const layerResults = await bridge.bridgeRunAllLayers();
            for (const [layer, layerResult] of Object.entries(layerResults)) {
              results.push({ step: layer, ok: true, via: 'bridge-fallback', output: JSON.stringify(layerResult).slice(0, 300) });
            }
          }
        } catch (bridgeErr) {
          results.push({ step: 'consolidate-all', ok: false, error: `MCP: ${mcpErr.message}. Bridge: ${bridgeErr.message}` });
        }
      }

      // Step 3: SONA learning via MCP
      try {
        const r = await vizMcpCall('hooks_intelligence_learn', {});
        results.push({ step: 'intelligence-learn', ok: true, via: 'mcp', output: JSON.stringify(r).slice(0, 300) });
      } catch (e) {
        results.push({ step: 'intelligence-learn', ok: false, error: e.message });
      }

      // Step 4: T1 intelligence consolidation (local, fast)
      const intFile = join(getDataRoot(), '.claude', 'helpers', 'intelligence.cjs');
      if (existsSync(intFile)) {
        try {
          const output = execSync(
            `node -e "const i=require('${intFile.replace(/'/g, "\\'")}');const r=i.consolidate?i.consolidate():null;console.log(JSON.stringify(r||{}))"`,
            { timeout: 10_000, encoding: 'utf8', cwd: getDataRoot() }
          );
          results.push({ step: 'T1-consolidate', ok: true, via: 'local', output: output.trim() });
        } catch (e) {
          results.push({ step: 'T1-consolidate', ok: false, error: e.message });
        }
      }

      res.json({ ok: results.every(r => r.ok), results, timestamp: new Date().toISOString() });
    } catch (e) {
      res.json({ ok: false, error: e.message, results });
    }
  });

  // ─── 4e. POST /api/daemon/control — Start/stop individual daemons ──
  ...
  app.get('/api/intelligence', (req, res) => {
    try {
      ...
      const rc = readJson('.claude-flow/data/ranked-context.json');

      let nodes = [], edges = [], pageRanks = {};
      if (gs) {
        nodes = Object.entries(gs.nodes || {}).map(([id, n]) => ({ id, ...n }));
        edges = gs.edges || [];
        pageRanks = gs.pageRanks || {};
      }

      const sorted = [...nodes].sort((a, b) => (pageRanks[b.id] || 0) - (pageRanks[a.id] || 0));

      res.json({
        graphState: {
          nodeCount: gs?.nodeCount || 0,
          edgeCount: edges.length,
          nodes: sorted.slice(0, 50),
          edges: edges.slice(0, 100),
          topPageRank: sorted.slice(0, 10).map(n => ({ id: n.id, score: pageRanks[n.id] || 0 })),
        },
        rankedContext: {
          entries: rc?.entries || [],
          version: rc?.version,
  ...
  // ─── 7. GET /api/session — Session state + metrics + exports ─

  // Reads from REAL sources:
  //   .claude-flow/sessions/current.json  — live session snapshot
  //   .swarm/memory.db → sessions table   — session history
  //   .claude-flow/daemon-state.json      — worker execution history
  //   .claude-flow/data/intelligence-snapshot.json — learning snapshots

  app.get('/api/session', (req, res) => {
    try {
      // Current session from Ruflo session file
      const current = readJson('.claude-flow/sessions/current.json');

      // Session history from memory.db
      const db = openDb('.swarm/memory.db');
      let sessions = [], memoryStats = {};
      let episodes = [], policies = [], learningSessions = [];
      let hierarchical = { working: 0, episodic: 0, semantic: 0 };
      if (db) {
        try { sessions = db.prepare('SELECT * FROM sessions ORDER BY rowid DESC LIMIT 20').all(); } catch {}
        try {
          const meta = db.prepare('SELECT key, value FROM metadata').all();
          memoryStats = Object.fromEntries(meta.map(r => [r.key, r.value]));
        } catch {}
        // Episodes (ReflexionMemory — success/failure + critique)
        try { episodes = db.prepare('SELECT task, success, reward, critique, created_at FROM episodes ORDER BY rowid DESC LIMIT 50').all(); } catch {}
        // Learning policies (trained RL models)
        try { policies = db.prepare('SELECT id, session_id, version, created_at FROM learning_policies ORDER BY created_at DESC LIMIT 10').all(); } catch {}
        // Learning sessions (RL lifecycle)
        try { learningSessions = db.prepare('SELECT id, status, start_time, end_time FROM learning_sessions ORDER BY start_time DESC LIMIT 10').all(); } catch {}
        // Hierarchical memory tiers
        try {
          const tiers = db.prepare('SELECT tier, COUNT(*) as c FROM hierarchical_memory GROUP BY tier').all();
          for (const t of tiers) hierarchical[t.tier] = t.c;
        } catch {}
        db.close();
      }

      // Daemon worker history
      const daemonState = readJson('.claude-flow/daemon-state.json');
      const workers = daemonState?.workers ? Object.entries(daemonState.workers).map(([name, w]) => ({
        name, runs: w.runCount || 0, successes: w.successCount || 0, failures: w.failureCount || 0,
        lastRun: w.lastRun, lastSuccess: w.lastSuccess, enabled: w.enabled !== false,
      })) : [];

      // Intelligence snapshots (learning progress over time)
      const snapshotPath = resolve(getDataRoot(), '.claude-flow', 'data', 'intelligence-snapshot.json');
      const snapshotExists = existsSync(snapshotPath);
      const snapshots = snapshotExists ? readJson('.claude-flow/data/intelligence-snapshot.json') : null;

      // Enrich session metadata with process-level info when file data is sparse
      const sessionMeta = current ? { ...current } : {};
      if (!sessionMeta.cwd) sessionMeta.cwd = process.cwd();
      if (!sessionMeta.startedAt) sessionMeta.startedAt = new Date().toISOString();
      if (!sessionMeta.id) sessionMeta.id = `tf-${Date.now().toString(36)}`;
      ...
      res.json({
        ...
        hierarchical,
        workers,
        snapshots: Array.isArray(snapshots) ? snapshots.slice(-20) : [],
        snapshotStatus: snapshotExists
          ? { exists: true }
          : { exists: false, message: 'Not yet generated \u2014 run session-end to create' },
        memoryStats,
        counts: {
  ...
  // ─── 7b. GET /api/current-session — Live session state from JSON file ──
  ...
  app.get('/api/agentdb', (req, res) => {
    try {
      // Primary: .swarm/memory.db (Ruflo ReasoningBank)
      const db = openDb('.swarm/memory.db');
      if (!db) return res.json({ tables: {}, summary: 'memory.db not found', tableCount: 0, totalRows: 0 });
      const tables = listTables(db);
      const result = {};
      for (const t of tables) {
        if (t.startsWith('sqlite_')) continue;
        result[t] = { count: countRows(db, t), schema: tableSchema(db, t), preview: tablePreview(db, t, 20) };
      }
      db.close();

      // Secondary: .claude/memory.db (Claude local)
      const claudeDb = openDb('.claude/memory.db');
      let claudeTables = {};
      if (claudeDb) {
        for (const t of listTables(claudeDb)) {
          if (t.startsWith('sqlite_')) continue;
          claudeTables[t] = { count: countRows(claudeDb, t) };
        }
        claudeDb.close();
      }
      ...
      res.json({
        ...
        totalRows: Object.values(result).reduce((s, t) => s + t.count, 0),
        claudeMemory: { tables: claudeTables, totalRows: Object.values(claudeTables).reduce((s, t) => s + t.count, 0) },
        hnsw: {
          metadata: hnswMeta,
          indexSize: hnswStat.size,
          indexExists: hnswStat.exists,
          lastModified: hnswStat.mtime,
        },
  ...
  // ─── 18. GET /api/hnsw — HNSW index health ──────────────────

  app.get('/api/hnsw', (req, res) => {
    try {
      const hnswMeta = readJson('.swarm/memory.hnsw.mappings.json');
      const hnswStat = fileStat('.swarm/memory.hnsw');
      const ruvectorStat = fileStat('ruvector.db');
      const memoryDbStat = fileStat('.swarm/memory.db');
      const agentdbStat = fileStat('.swarm/agentdb-memory.db');
      const memoryGraphStat = fileStat('.swarm/memory.graph');
      let vectorCount = 0;
      const db = openDb('.swarm/memory.db');
      if (db) {
        for (const t of listTables(db)) {
          if (t.includes('vector') || t.includes('embedding') || t.includes('memory')) {
            vectorCount += countRows(db, t);
          }
        }
        db.close();
      }
      res.json({
        index: {
          exists: hnswStat.exists || ruvectorStat.exists,
          size: hnswStat.size || ruvectorStat.size,
          sizeHuman: (hnswStat.size || ruvectorStat.size) ? ((hnswStat.size || ruvectorStat.size) / 1024).toFixed(1) + ' KB' : '0',
          lastModified: hnswStat.mtime || ruvectorStat.mtime,
        },
        metadata: hnswMeta || {},
        vectorCount,
        nativeRedb: { exists: ruvectorStat.exists, size: ruvectorStat.size, lastModified: ruvectorStat.mtime },
        stores: {
          memoryDb: { exists: memoryDbStat.exists, size: memoryDbStat.size },
          agentDb: { exists: agentdbStat.exists, size: agentdbStat.size },
          memoryGraph: { exists: memoryGraphStat.exists, size: memoryGraphStat.size, lastModified: memoryGraphStat.mtime },
        },
        timestamp: new Date().toISOString(),
      });
    } catch (err) {
  ...
  // ─── 19. GET /api/sona — SONA patterns + EWC Fisher ─────────

  // SONA/EWC — reads from REAL .swarm/memory.db tables:
  //   trajectories       — SONA trajectory tracking
  ...
  // ─── 20. GET /api/auto-memory — Auto-memory store ────────────
  ...
  app.get('/api/insights-queue', (req, res) => {
    try {
      const insightsPath = resolvePath('.claude-flow/data/pending-insights.jsonl');
      const stat = fileStat('.claude-flow/data/pending-insights.jsonl');
      let lines = [], totalCount = 0;
      if (stat.exists) {
        const content = readFileSync(insightsPath, 'utf8');
        const rawLines = content.trim().split('\n').filter(Boolean);
        totalCount = rawLines.length;
        lines = rawLines.slice(-50).map(line => {
          try { return JSON.parse(line); } catch { return { raw: line }; }
        });
      }
      const byType = {};
      for (const l of lines) { const t = l.type || l.category || 'unknown'; byType[t] = (byType[t] || 0) + 1; }
      res.json({
        exists: stat.exists, totalCount, recentInsights: lines, byType,
        fileSize: stat.size, lastModified: stat.mtime,
        timestamp: new Date().toISOString(),
      });
    } catch (err) {
  ...
  // ─── 22. GET /api/mcp-health — MCP server connectivity ───────

  // MCP Health — checks .mcp.json (stdio servers) AND Ruflo framework tools
  // In this project, MCP comes via Ruflo (59+ tools), not stdio servers in .mcp.json
  ...
  app.get('/api/ewc-status', (req, res) => {
    try {
      const fisherState = readJsonSafe('.swarm/ewc-fisher.json', null);

      // Count ewc_patterns rows in memory.db
      let patternCount = 0;
      const db = openDb('.swarm/memory.db');
      if (db) {
        patternCount = countRows(db, 'ewc_patterns');
        db.close();
      }

      // Determine last consolidation from fisher state or file mtime
      let lastConsolidation = null;
      if (fisherState?.lastConsolidation) {
        lastConsolidation = fisherState.lastConsolidation;
      } else {
        const stat = fileStat('.swarm/ewc-fisher.json');
        if (stat.exists && stat.mtime) lastConsolidation = stat.mtime;
      }

      res.json({ fisherState, patternCount, lastConsolidation });
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
  });

  // ─── P3.1-2. GET /api/embeddings-config — Embeddings configuration ─
  ...
  app.get('/api/rewards', (req, res) => {
    try {
      ...
      // ── Captured patterns from SQL (.swarm/memory.db) ──
      let rows = [];
      let capturedCount = 0;
      let capturedByType = {};
      const db = openDb('.swarm/memory.db');
      if (db) {
        ...
        // reasoning_patterns (all columns safe, has timestamps)
        try {
          const patterns = db.prepare(`SELECT id, task_type AS action, success_rate AS reward, 1 AS success, ts * 1000 AS timestamp FROM reasoning_patterns`).all();
          rows = patterns;
          capturedCount = patterns.length;
          // Group by task_type for breakdown
          for (const p of patterns) {
            capturedByType[p.action] = (capturedByType[p.action] || 0) + 1;
          }
        } catch {}

        // Supplement: learning_experiences (only id+reward safe — TEXT columns have corrupt overflow pages)
        if (tables.includes('learning_experiences')) {
          try {
            const le = db.prepare(`SELECT id, reward FROM learning_experiences`).all();
            const now = Date.now();
            const span = le.length > 1 ? 3600000 : 0;
            le.forEach((r, i) => {
              rows.push({ id: 1000 + r.id, action: 'learning_experience', reward: r.reward, success: 1, timestamp: now - span + (i / le.length) * span });
            });
          } catch {}
        }
        db.close();
      ...
      // Coerce reward to number (SQLite may store as TEXT)
      for (const r of rows) r.reward = Number(r.reward) || 0;
      if (since > 0) rows = rows.filter(r => (r.timestamp || 0) > since);
      rows.sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0));
      rows = rows.slice(0, limit);

      // ── Learned patterns from neural JSON (.claude-flow/neural/patterns.json) ──
      let learned = { count: 0, patterns: [], confidenceRange: [0, 0], byType: {} };
      const neuralData = readJsonSafe('.claude-flow/neural/patterns.json', null);
      if (neuralData) {
        const pats = Array.isArray(neuralData) ? neuralData
          : neuralData.patterns ? (Array.isArray(neuralData.patterns) ? neuralData.patterns : [])
          : [];
        if (pats.length) {
          const confs = pats.map(p => Number(p.confidence) || 0);
          learned.count = pats.length;
          learned.confidenceRange = [Math.min(...confs), Math.max(...confs)];
          // By patternType
          for (const p of pats) {
            const pt = (p.metadata && p.metadata.patternType) || 'unknown';
            learned.byType[pt] = (learned.byType[pt] || 0) + 1;
          }
          ...
          learned.confidenceBuckets = buckets;
          // Summary per pattern (no embeddings)
          learned.patterns = pats.map(p => ({
            id: p.id,
            content: p.content,
            confidence: p.confidence,
            type: (p.metadata && p.metadata.patternType) || 'unknown',
      ...
      res.json({
        rewards: rows,
        count: rows.length,
        captured: { count: capturedCount, byType: capturedByType },
        learned,
      });
    } catch (err) {
      res.status(500).json({ error: err.message });
  ...
  // ─── GET /api/trajectories — Trajectory timeline data ──

  app.get('/api/trajectories', async (req, res) => {
    try {
      const db = openDb('.swarm/memory.db');
      if (!db) return res.json({ trajectories: [] });
      const tables = listTables(db);
      if (!tables.includes('trajectories')) {
        db.close();
        return res.json({ trajectories: [], note: 'trajectories table not found' });
      }

      const sessionId = req.query.session_id;
      const limit = parseInt(req.query.limit) || 50;

      const hasStepsTable = tables.includes('trajectory_steps');
      let query = `SELECT t.id, t.session_id, t.status, t.verdict, t.total_steps, t.total_reward, t.task, t.started_at, t.ended_at${
        hasStepsTable ? ', (SELECT COUNT(*) FROM trajectory_steps ts WHERE ts.trajectory_id = t.id) AS actual_steps' : ''
      } FROM trajectories t`;
      const params = [];
      if (sessionId) { query += ' WHERE t.session_id = ?'; params.push(sessionId); }
      query += ' ORDER BY t.started_at DESC LIMIT ?';
      params.push(limit);

      const trajs = db.prepare(query).all(...params);

      // Compute dominantModel per trajectory from JSONL transcript (real model per assistant message)
      // Step metadata is unreliable (always records hook context "sonnet").
      try {
        const jsonl = findLatestJsonl();
        if (jsonl) {
          const content = readFileSync(jsonl.path, 'utf8');
          // Extract (timestamp, model) pairs from assistant messages
          const modelEvents = [];
          for (const line of content.split('\n')) {
            if (!line || !line.includes('"assistant"')) continue;
            try {
              const obj = JSON.parse(line);
              if (obj.message?.role === 'assistant' && obj.message?.model) {
                const ts = obj.timestamp ? new Date(obj.timestamp).getTime() : 0;
                // Normalize model ID: "claude-opus-4-6" → "opus", "claude-sonnet-4-6" → "sonnet", etc.
                const raw = obj.message.model;
                const mdl = raw.includes('opus') ? 'opus' : raw.includes('sonnet') ? 'sonnet' : raw.includes('haiku') ? 'haiku' : raw;
                if (ts > 0) modelEvents.push({ ts, model: mdl });
              }
            } catch {}
          }
          modelEvents.sort((a, b) => a.ts - b.ts);

          for (const t of trajs) {
            const tStart = t.started_at ? new Date(t.started_at).getTime() : 0;
            const tEnd = t.ended_at ? new Date(t.ended_at).getTime() : Date.now();
            if (!tStart) continue;
            // Find model events within this trajectory's time range
            const counts = {};
            for (const ev of modelEvents) {
              if (ev.ts >= tStart && ev.ts <= tEnd) {
            ...
            if (entries.length) {
              ...
              t.dominantModel = entries[0][0];
              t.modelCounts = Object.fromEntries(entries);
              t.modelSource = 'transcript';
            }
          }
        }
      } catch {}

      // Enrich with routing level from hook-activity.jsonl route events
      try {
        const actPath = resolvePath('.claude-flow/data/hook-activity.jsonl');
        if (existsSync(actPath)) {
          const routeEvents = [];
          for (const line of readFileSync(actPath, 'utf8').split('\n')) {
            ...
            try {
              ...
              if (ev.e === 'route' && ev.ts) {
                const calls = ev.c || [];
                // Determine routing level from rl field or infer from calls
                let rl = ev.rl;
                if (!rl) {
                  if (calls.includes('agentdb_semantic-route'))      rl = 'agentdb-semanticRouter';
                  else if (calls.includes('hooks_route'))            rl = 'keyword';
                  else                                               rl = 'default';
                }
                routeEvents.push({ ts: ev.ts, rl });
              }
            } catch {}
          }
          routeEvents.sort((a, b) => a.ts - b.ts);
          // Match route events to trajectories by timestamp overlap
          for (const t of trajs) {
            const tStart = t.started_at ? new Date(t.started_at).getTime() : 0;
            const tEnd = t.ended_at ? new Date(t.ended_at).getTime() : Date.now();
            if (!tStart) continue;
            const matched = routeEvents.filter(r => r.ts >= tStart && r.ts <= tEnd);
            if (matched.length) {
              // Pick highest-priority (lowest level number) route seen
              const priority = ['agentdb-semanticRouter','sona-native','sona-pattern','q-learning','keyword','default'];
              let best = matched[0].rl;
              for (const m of matched) {
                if (priority.indexOf(m.rl) < priority.indexOf(best) && priority.indexOf(m.rl) !== -1) {
                  best = m.rl;
                }
              }
              t.routingLevel = best;
              t.routingCount = matched.length;
            }
          }
        }
      } catch {}
  ...
  app.get('/api/trajectories/:id/steps', (req, res) => {
    try {
      const db = openDb('.swarm/memory.db');
      if (!db) return res.json({ steps: [] });
      const tables = listTables(db);
      if (!tables.includes('trajectory_steps')) {
        db.close();
        return res.json({ steps: [], note: 'trajectory_steps table not found' });
      }

      let steps, corrupted = false;
      try {
        // Full query with TEXT columns (action, observation, metadata)
        steps = db.prepare(
          'SELECT step_number, action, observation, reward, metadata, created_at FROM trajectory_steps WHERE trajectory_id = ? ORDER BY step_number'
        ).all(req.params.id);
        for (const s of steps) {
          try { s.meta = JSON.parse(s.metadata); } catch { s.meta = null; }
        }
      } catch {
        // Fallback: B-tree pages may be corrupt — full table scan + JS filter
        corrupted = true;
        try {
          const all = db.prepare('SELECT trajectory_id, step_number, reward FROM trajectory_steps').all();
          steps = all.filter(s => s.trajectory_id === req.params.id);
          steps.sort((a, b) => a.step_number - b.step_number);
          for (const s of steps) { s.action = '(data unavailable)'; s.observation = ''; s.meta = null; delete s.trajectory_id; }
        } catch {
          steps = [];
        }
      }
      db.close();
      res.json({ steps, corrupted });
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
  });

  // ─── GET /api/routing-stats — ModelRouter stats (ADR-LOCAL-003) ──
  ...
  // ─── GET /api/architecture-live — Learning cycle ring diagram data ──
  // Single poll returns all 6 node counts + session state + Q-values + ModelRouter stats

  app.get('/api/architecture-live', async (req, res) => {
    try {
      const result = {
        nodes: { route: {}, execute: {}, capture: {}, store: {}, learn: {}, recall: {} },
        session: {},
        modelRouter: {},
        improvement: null,
        timestamp: new Date().toISOString(),
      ...
      const sessPath = join(getDataRoot(), '.claude-flow', 'data', 'current-session.json');
      if (existsSync(sessPath)) {
        try { result.session = JSON.parse(readFileSync(sessPath, 'utf-8')); } catch {}
      }
      ...
      } catch { result.modelRouter = { available: false }; }

      // Latest [TASK_MODEL_RECOMMENDATION] from transcript (may include ast_complexity)
      try {
        const rec = getLatestModelRecommendation();
        if (rec) {
          result.modelRouter.latestRecommendation = rec;
          if (rec.astComplexity != null) result.modelRouter.astComplexity = rec.astComplexity;
          if (rec.file) result.modelRouter.routedFile = rec.file;
        }
      } catch {}
      ...
      const db = openDb('.swarm/memory.db');
      if (db) {
        // ROUTE node: routing decisions + model info
        result.nodes.route = {
          model: result.session.model || 'unknown',
          routingReason: result.session.routingReason || '',
          sessionGoal: result.session.sessionGoal || '',
          totalDecisions: result.modelRouter.totalDecisions || 0,
          modelDistribution: result.modelRouter.modelDistribution || {},
          circuitBreakerTrips: result.modelRouter.circuitBreakerTrips || 0,
        };

        // EXECUTE node: active trajectories
        try {
          const active = db.prepare("SELECT status, total_steps, task FROM trajectories WHERE status='active' LIMIT 1").get();
          const totalTrajectories = db.prepare("SELECT COUNT(*) as c FROM trajectories").get()?.c || 0;
          result.nodes.execute = {
            activeTask: active?.task || null,
            activeSteps: active?.total_steps || 0,
            activeStatus: active?.status || 'none',
            totalTrajectories,
          };
        } catch { result.nodes.execute = { activeTask: null, activeSteps: 0, totalTrajectories: 0 }; }

        // CAPTURE node: trajectory_steps + reasoning_patterns
        try {
          const stepCount = db.prepare("SELECT COUNT(*) as c FROM trajectory_steps").get()?.c || 0;
          let errorPatterns = 0, coEditPatterns = 0;
          try { errorPatterns = db.prepare("SELECT COUNT(*) as c FROM reasoning_patterns WHERE task_type='error_recovery'").get()?.c || 0; } catch {}
          try { coEditPatterns = db.prepare("SELECT COUNT(*) as c FROM reasoning_patterns WHERE task_type='co_edit'").get()?.c || 0; } catch {}
          result.nodes.capture = { steps: stepCount, errors: errorPatterns, coEdits: coEditPatterns };
        } catch { result.nodes.capture = { steps: 0, errors: 0, coEdits: 0 }; }

        // STORE node: hierarchical_memory tiers + memory_entries + reasoning_patterns
        try {
          const tiers = { working: 0, episodic: 0, semantic: 0 };
          try {
            const rows = db.prepare("SELECT tier, COUNT(*) as c FROM hierarchical_memory GROUP BY tier").all();
            for (const r of rows) tiers[r.tier] = r.c;
          } catch {}
          const memEntries = db.prepare("SELECT COUNT(*) as c FROM memory_entries").get()?.c || 0;
          let reasoningPatterns = 0;
          try { reasoningPatterns = db.prepare("SELECT COUNT(*) as c FROM reasoning_patterns").get()?.c || 0; } catch {}
          result.nodes.store = { tiers, memEntries, reasoningPatterns };
        } catch { result.nodes.store = { tiers: {}, memEntries: 0, reasoningPatterns: 0 }; }

        // LEARN node: learning_policies + Q-values + episodes success count
        try {
          const policyCount = db.prepare("SELECT COUNT(*) as c FROM learning_policies").get()?.c || 0;
          let qValueCount = 0;
          try {
            const latest = db.prepare("SELECT q_values FROM learning_policies ORDER BY created_at DESC LIMIT 1").get();
            if (latest?.q_values) {
              const qv = JSON.parse(latest.q_values);
              qValueCount = Object.keys(qv).length;
            }
          } catch {}
          let successEpisodes = 0;
          try { successEpisodes = db.prepare("SELECT COUNT(*) as c FROM episodes WHERE success = 1").get()?.c || 0; } catch {}
          result.nodes.learn = { policies: policyCount, qValues: qValueCount, successEpisodes };
        } catch { result.nodes.learn = { policies: 0, qValues: 0, successEpisodes: 0 }; }

        // RECALL node: episodes + error patterns recalled + mailbox
        try {
          const episodeCount = db.prepare("SELECT COUNT(*) as c FROM episodes").get()?.c || 0;
          let errorsRecalled = 0;
          try { errorsRecalled = db.prepare("SELECT COUNT(*) as c FROM reasoning_patterns WHERE task_type='error_recovery'").get()?.c || 0; } catch {}
          let mailbox = 0;
          try { mailbox = db.prepare("SELECT COUNT(*) as c FROM memory_entries WHERE namespace='mailbox'").get()?.c || 0; } catch {}
          result.nodes.recall = { episodes: episodeCount, errorsRecalled, mailbox };
        } catch { result.nodes.recall = { episodes: 0, errorsRecalled: 0, mailbox: 0 }; }

        // Improvement indicator: compare first-half vs second-half sessions
        try {
          const episodes = db.prepare("SELECT reward FROM episodes ORDER BY created_at ASC").all();
          if (episodes.length >= 10) {
            const mid = Math.floor(episodes.length / 2);
            const firstHalf = episodes.slice(0, mid);
            const secondHalf = episodes.slice(mid);
            const avgFirst = firstHalf.reduce((s, e) => s + (e.reward || 0), 0) / firstHalf.length;
            const avgSecond = secondHalf.reduce((s, e) => s + (e.reward || 0), 0) / secondHalf.length;
            const rewardDelta = ((avgSecond - avgFirst) / (avgFirst || 1) * 100).toFixed(1);
            result.improvement = { rewardDelta, sessions: episodes.length, ready: true };
          } else {
            result.improvement = { ready: false, sessions: episodes.length };
          }
        } catch { result.improvement = { ready: false, sessions: 0 }; }

        // Revolutions: count completed trajectories this session (nautilus spiral growth)
        try {
          const sessId = result.session.sessionId || result.session.id;
          if (sessId) {
            result.revolutions = db.prepare(
              "SELECT COUNT(*) as c FROM trajectories WHERE session_id = ? AND status = 'completed'"
            ).get(sessId)?.c || 0;
          } else {
            result.revolutions = 0;
          }
        } catch { result.revolutions = 0; }

        db.close();
      }

      // Session summary for center panel
      result.center = {
        sessionId: result.session.sessionId || result.session.id || null,
        goal: result.session.sessionGoal || result.session.goal || '',
        steps: result.nodes.capture.steps,
        avgReward: result.improvement?.ready
          ? (parseFloat(result.improvement.rewardDelta) > 0 ? '+' : '') + result.improvement.rewardDelta + '%'
          : 'collecting...',
        model: result.nodes.route.model,
        complexity: result.modelRouter.avgComplexity ? result.modelRouter.avgComplexity.toFixed(0) + '%' : '',
        astComplexity: result.modelRouter.astComplexity != null ? result.modelRouter.astComplexity.toFixed(0) + '%' : '',
        routedFile: result.modelRouter.routedFile || '',
        qValues: result.nodes.learn.qValues,
        tierSummary: result.nodes.store.tiers
          ? `${result.nodes.store.tiers.working || 0}W > ${result.nodes.store.tiers.episodic || 0}E > ${result.nodes.store.tiers.semantic || 0}S`
          : '',
        mailbox: result.nodes.recall.mailbox,
        revolutions: result.revolutions || 0,
      };

      // Behavior indicators
      result.behaviors = [
        { id: 'model-router', label: 'EnhancedModelRouter', status: result.modelRouter.available ? 'active' : 'inactive', where: 'route' },
        { id: 'circuit-breaker', label: 'Circuit Breaker', status: (result.modelRouter.circuitBreakerTrips || 0) >= 0 ? 'active' : 'inactive', where: 'route' },
        { id: 'rl-predict', label: 'RL Predict', status: result.nodes.learn.qValues > 0 ? 'active' : 'inactive', where: 'learn-recall' },
        { id: 'worker-dispatch', label: 'Worker Dispatch', status: 'active', where: 'route' },
        { id: 'mailbox', label: 'Mailbox', status: result.nodes.recall.mailbox >= 0 ? 'active' : 'inactive', where: 'recall' },
        { id: 'critique', label: 'Critique/Errors', status: result.nodes.recall.errorsRecalled > 0 ? 'indirect' : 'inactive', where: 'recall-route' },
        { id: 'sona', label: 'SONA Suggestions', status: result.nodes.execute.totalTrajectories > 0 ? 'indirect' : 'inactive', where: 'recall-route' },
      ];
